...
- `inspect_verse_device` / `get_verse_editables` `STOP: true` or `readable: false` is **advisory**. Ladder: `get_verse_editables` on **this same device** → `readable: true`? wire once. Not readable? confirm `@editable` in the `.verse`, `workspace_compile_verse` once, wait (10054 = started), re-inspect the same device. Still not readable: `reload_listener` **once**, then stop and report. Never a second device, never a Verse refactor, never restart UEFN. Internals: `skill_read_subskill("uefn", "verse_editable_internals")`.
- `STOP: false` (even with `wiring.status: partial`) → wire with labels now.
- **STALE REFLECTION / `readable: false`:** the computed mangled name is not readable on this Script — (a) Verse build for this edit has not landed, (b) the field is not `@editable` in source, or (c) the Python wrapper is stale. **Do not call `wire_verse_*` again.** Host already compiles + reloads + retries once. Confirm `@editable` in the `.verse`, wait out `WinError 10054`, poll `list_verse_types`, then `get_verse_editables` on the **same** device and wire once. **Never place a second copy. Never remove or retype an `@editable` to dodge this.** One Verse class = one placed device unless the design needs more. First array item: `wire_verse_device_array` + `target_paths=[one]`. Rewrite: same device, `replace=true`, full list. `skill_read_subskill("uefn", "verse_devices")`.
...
| Scan `.uasset` / binaries / `os.walk` for `__verse_0x` hashes | Direct read/write of one object's mangled `__verse_0x<HASH>_<Field>` is fine; prefer `get_verse_editables` / `wire_verse_*`. The hash is computed from the field name — never scan disk. |
