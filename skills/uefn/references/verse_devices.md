...
| Spawn | `spawn_actor` → wait → `set_actor_label` (once per device) |
...
`search_assets(directory="/_Verse")` for `asset_path` → `spawn_actor` → wait →
`set_actor_label` → `wire_verse_device_ref` **one field per turn** →
